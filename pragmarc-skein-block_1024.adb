...
with PragmARC.UBI.Block_1024;
with PragmARC.Encryption.Threefish.Block_1024;

package body PragmARC.Skein.Block_1024 is
   subtype Block is UBI.Block_1024.Block;
   ...
   function Hash (Message : in Byte_List; Num_Bytes : in Positive) return Byte_List is
      ...
      function Output (Key : in Block) return Byte_List is
         subtype Block_As_Bytes is Encryption.Threefish.Block_1024.Block_As_Bytes;
      ...
      begin -- Output
         Fill : for I in Word loop
            Bunch := Encryption.Threefish.Block_1024.Bytes_From_Block
                        (UBI.Block_1024.Compress (Encryption.Threefish.Bytes_From_Word (I), Key, Tweak) );
   ...
   begin -- Hash
      Key := UBI.Block_1024.Compress (Config, Key, Tweak);
      Tweak (1) := 48 * 2 ** 56; -- Type set to Message
      Key := UBI.Block_1024.Compress (Message, Key, Tweak);
