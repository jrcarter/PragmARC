-- PragmAda Reusable Component (PragmARC)
-- Copyright (C) by PragmAda Software Engineering
-- SPDX-License-Identifier: BSD-3-Clause
-- See https://spdx.org/licenses/
-- If you find this software useful, please let me know, either through
-- github.com/jrcarter or directly to pragmada@pragmada.x10hosting.com
-- **************************************************************************
--
-- Root of the Data_Structures.Bags.Unbounded hierarchy
--
-- History:
-- 2025 Jul 01     J. Carter          V1.2--Use SPDX license format
-- 2020 Dec 01     J. Carter          V1.1--Changed elaboration pragmas to aspects
-- 2020 Nov 01     J. Carter          V1.0--Initial Ada-12 version
--
package PragmARC.Data_Structures.Bags.Unbounded with Pure is
   -- Empty
end PragmARC.Data_Structures.Bags.Unbounded;
