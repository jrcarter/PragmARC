...
package body PragmARC.UBI.Block_256 is
   function Compress (Message : in Byte_List; Init_Key : in Block; Init_Tweak : in Couple) return Block is
      subtype Block_List is Encryption.Threefish.Block_256.Block_List;
      use type Block_List;

      Bytes_Per_Block : constant Positive := Encryption.Threefish.Block_256.Block_As_Bytes'Length;

      function To_Block_List (Message : in Byte_List) return Block_List is
         (if Message'Length = 0 then (1 .. 0 => <>)
          else Encryption.Threefish.Block_256.Block_From_Bytes (Message (Message'First .. Message'First + Bytes_Per_Block - 1) ) &
      ...
      Key    : Block;
      KS     : Encryption.Threefish.Block_256.Key_Schedule_Handle;
   begin -- Compress
      All_Blocks : for K in M'Range loop
         ...
         Encryption.Threefish.Block_256.Create_Key_Schedule (Key => Key, Tweak => Tweak, Key_Schedule => KS);
         Result := M (K);
         Encryption.Threefish.Block_256.Encrypt (Key_Schedule => KS, Text => Result);
         Result := Result xor M (K);
      end loop All_Blocks;

      return Result;
   end Compress;
end PragmARC.UBI.Block_256;
