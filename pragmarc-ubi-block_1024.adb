...
package body PragmARC.UBI.Block_1024 is
   function Compress (Message : in Byte_List; Init_Key : in Block; Init_Tweak : in Couple) return Block is
      subtype Block_List is Encryption.Threefish.Block_1024.Block_List;
      use type Block_List;

      Bytes_Per_Block : constant Positive := Encryption.Threefish.Block_1024.Block_As_Bytes'Length;

      function To_Block_List (Message : in Byte_List) return Block_List is
         (if Message'Length = 0 then (1 .. 0 => <>)
          else Encryption.Threefish.Block_1024.Block_From_Bytes
                  (Message (Message'First .. Message'First + Bytes_Per_Block - 1) ) &
      ...
      function "xor" (Left : in Block; Right : in Block) return Block is
         (Left ( 0) xor Right ( 0), Left ( 1) xor Right ( 1), Left ( 2) xor Right ( 2), Left ( 3) xor Right ( 3),
          ...
          Left ( 8) xor Right ( 8), Left ( 9) xor Right ( 9), Left (10) xor Right (10), Left (11) xor Right (11),
          Left (12) xor Right (12), Left (13) xor Right (13), Left (14) xor Right (14), Left (15) xor Right (15) );
      ...
      Key    : Block;
      KS     : Encryption.Threefish.Block_1024.Key_Schedule_Handle;
   begin -- Compress
      All_Blocks : for K in M'Range loop
         ...
         Encryption.Threefish.Block_1024.Create_Key_Schedule (Key => Key, Tweak => Tweak, Key_Schedule => KS);
         Result := M (K);
         Encryption.Threefish.Block_1024.Encrypt (Key_Schedule => KS, Text => Result);
